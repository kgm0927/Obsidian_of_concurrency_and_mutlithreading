...
좀 더 일반적으로는 소위 바쁜-대기(busy-wait)원리를 사용한다. 이것은 프로세서가 데이터를 패치하는 방식 즉즉, 단 하나의 태스크만이 공유 메모리 내의 원자적 값(CPU 레지스터와 동일한 크기[또는 더 작은]의 변수)를 구하여 읽기-쓰기를 할 수 있음으로 인해 뮤텍스 이면에 있는 기본 원리이다. (이 원리는 8장에 나온다.)
